...
export class ConsumerLeadListComponent implements OnInit {
  ...
  constructor(public serve:PearlService,public dialog:DialogComponent,public session: LocalStorage) {
    ...
    console.log("User Id :",this.login_data_2);
  ...
  leadList(type : any = ''){
    ...
    this.serve.fetchData({'start':this.start,'pagelimit':this.page_limit,'search':this.search_val ,'user_id':this.login_data_2.id , 'user_type': this.login_data_2.user_type},"Distributors/direct_dealer_lead_list")
    .subscribe((result=>{
      console.log(result);
  ...
  deleteLead(id)
  {
    ...
    exportAsXLSX():void {
      ...
      this.serve.exportAsExcelFile(this.excel_data, 'Direct Dealer LEAD SHEET');
    }
    ...
    download_all_branding(){
      ...
      this.serve.fetchData({'search':this.search_val,'type':1,'lead_type':'Lead', 'user_id':this.login_data_2.id , 'user_type': this.login_data_2.user_type},"Distributors/get_all_branding_data_excel").subscribe((result=>{
          console.log(result);
