...
export class SocialMediaLeadListComponent implements OnInit {
  ...
  leadList(type : any = ''){
    ...
    this.serve.fetchData({'start':this.start,'pagelimit':this.page_limit,'search':this.search_val ,'user_id':this.login_data_2.id , 'user_type': this.login_data_2.user_type},"Distributors/oem_lead_list")
    .subscribe((result=>{
      console.log(result);
